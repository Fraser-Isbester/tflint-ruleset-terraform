...
func (validator *NameValidator) checkBlock(runner tflint.Runner, r *TerraformNamingConventionRule, blockTypeName string, blockName string, blockDeclRange *hcl.Range) error {
	if validator != nil && !validator.Regexp.MatchString(blockName) {
		...
		message := fmt.Sprintf("%s name `%s` must match the following %s: %s", blockTypeName, blockName, formatType, validator.Format)

		// Only auto-fix for named formats and resource blocks
		if validator.IsNamedFormat && blockTypeName == "resource" {
			var fixedName string
			switch validator.Format {
			case "snake_case":
				fixedName = toSnakeCase(blockName)
			case "mixed_snake_case":
				fixedName = toMixedSnakeCase(blockName)
			default:
				// For custom formats or none, just emit the issue
				return runner.EmitIssue(r, message, *blockDeclRange)
			}
			...
			return runner.EmitIssueWithFix(
				...
				*blockDeclRange,
				func(f tflint.Fixer) error {
					// Create the new resource declaration with the fixed name
					newResourceDecl := strings.Replace(
						blockName,
						blockName,
						fixedName,
						1,
					...
					if err := f.ReplaceText(
						hcl.Range{
							Filename: blockDeclRange.Filename,
							Start: hcl.Pos{
								Line:   blockDeclRange.Start.Line,
								Column: blockDeclRange.Start.Column,
								Byte:   blockDeclRange.Start.Byte,
							},
							End: hcl.Pos{
								Line:   blockDeclRange.End.Line,
								Column: blockDeclRange.End.Column,
								Byte:   blockDeclRange.End.Byte,
							},
						},
						newResourceDecl,
					); err != nil {
					...
					// Add the moved block
					movedBlock := fmt.Sprintf("\n\n# Auto-generated by tflint. Safe to remove after apply.\nmoved {\n  from = %s.%s\n  to = %s.%s\n}",
						blockTypeName, blockName,
						blockTypeName, fixedName,
					)

					return f.InsertTextAfter(*blockDeclRange, movedBlock)
				},
			)
		}

		// For non-resource blocks or custom regex formats, just emit the issue
		return runner.EmitIssue(r, message, *blockDeclRange)
	}
	return nil
}
...
func toSnakeCase(s string) string {
	...
	for i, r := range s {
		if i > 0 && unicode.IsUpper(r) {
			...
			// or if next char exists and is lowercase
	...
	return strings.Trim(strings.ReplaceAll(result.String(), "__", "_"), "_")
}

// toMixedSnakeCase converts a string to MixedSnakeCase format
func toMixedSnakeCase(s string) string {
	// Handle common separators
	s = strings.ReplaceAll(s, "-", "_")
	s = strings.ReplaceAll(s, " ", "_")
